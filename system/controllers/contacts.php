...
switch ($action) {
    case 'messages':
        $ui->assign('_application_menu', 'messages');
        $ui->assign('_title', $_L['List Messages']);
        $db_con = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
        mysqli_set_charset($db_con, 'utf8');

        $ds = new MySQLiDataSource($db_con);
        $ds->SelectCommand = "select id as message_id,content from messages";
        $ds->UpdateCommand = "update messages set content='@content' where id = @message_id";
        $ds->DeleteCommand = "delete from messages where id=@message_id;";
        $ds->InsertCommand = "insert into messages (content) values ('@content');";

        $ds_mapping = new MySQLiDataSource($db_con);
        $ds_mapping->SelectCommand = "select message_user_assoc.id as assoc_id,message_id, user_id from message_user_assoc";
        $ds_mapping->UpdateCommand = "update message_user_assoc set user_id='@user_id' where id = @assoc_id";
        $ds_mapping->DeleteCommand = "delete from message_user_assoc where id=@assoc_id";
        $ds_mapping->InsertCommand = "insert into message_user_assoc (user_id,message_id) values ('@user_id', '@message_id');";
...
//        $grid->AllowScrolling = true;
        ...
        $table_mapping->AddRelationField("message_id", "message_id");
//        $table_mapping->AutoGenerateExpandColumn = true;
        $table_mapping->DisableAutoGenerateDataFields = "message_id";
        ...
        $column->DataField = "user_id";
        $column->HeaderText = $_L['Full_Name'];
        $names = ORM::for_table('crm_accounts')->select('account')->select('id')->where('type', 'Customer')->find_array();
        foreach ($names as $item) {
            $column->AddItem($item['account'],$item['id']);
        }
        ...
        $column->HeaderText = $_L['Message Content'];
        $grid->MasterTable->AddColumn($column);
