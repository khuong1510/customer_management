...
switch ($action) {
    case 'videos':
        $ui->assign('_application_menu', 'videos');
        $ui->assign('_title', $_L['List Videos']);
//        $ui->assign('_url_import', "tree/import_csv");
        ...
        $ds->SelectCommand = "select id,link from videos";
        $ds->UpdateCommand = "update videos set link='@link' where id = @id";
        $ds->DeleteCommand = "delete from videos where id=@id";
        $ds->InsertCommand = "insert into videos (link) values ('@link');";
...
//        $grid->Width = "100%";
    ...
    case 'questions':
        $ui->assign('_application_menu', 'questions');
        $ui->assign('_title', $_L['List Questions']);
        $db_con = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
        mysqli_set_charset($db_con, 'utf8');

        $ds = new MySQLiDataSource($db_con);
        $ds->SelectCommand = "select id,content, tree_list from questions";
        $ds->UpdateCommand = "update questions set content='@content', tree_list='@tree_list' where id = @id";
        $ds->DeleteCommand = "delete from questions where id=@id;delete from answers where question_id=@id;";
        $ds->InsertCommand = "insert into questions (content, tree_list) values ('@content', '@tree_list');";

        $ds_mapping = new MySQLiDataSource($db_con);
        $ds_mapping->SelectCommand = "select answers.id as answer_id,question_id,content from answers ";
        $ds_mapping->UpdateCommand = "update answers set content='@content' where id = @answer_id";
        $ds_mapping->DeleteCommand = "delete from answers where id=@answer_id";
        $ds_mapping->InsertCommand = "insert into answers (content,question_id) values ('@content', '@question_id');";
...
//        $grid->AllowScrolling = true;
        ...
        $table_mapping = new GridTableView();
        $table_mapping->Width = "100%";
        $table_mapping->DataSource = $ds_mapping;
        $table_mapping->AddRelationField("question_id", "id");
//        $table_mapping->AutoGenerateExpandColumn = true;
        $table_mapping->DisableAutoGenerateDataFields = "question_id";
        $table_mapping->AllowEditing = true;
        $table_mapping->AllowDeleting = true;
        $table_mapping->AllowInserting = true;
        $table_mapping->EditSettings->Mode = "Inline";
        //Show Function Panel
        $table_mapping->ShowFunctionPanel = true;
        //Insert Settings
        ...
        $column->HeaderText = $_L['Answer Content'];
        ...
        $column->HeaderText = $_L['Action'];
        $table_mapping->AddColumn($column);
        // End Answer Mapping Columns

        $grid->MasterTable->DisableAutoGenerateDataFields = "id";
        $grid->MasterTable->DataSource = $ds;
        $grid->MasterTable->AllowFiltering = true;
        $grid->MasterTable->AutoGenerateExpandColumn = true;
        $grid->MasterTable->AddDetailTable($table_mapping);
        $grid->MasterTable->Pager = new GridPrevNextAndNumericPager();
        $grid->MasterTable->EditSettings->Mode = "Inline";
        ...
        $column = new GridTextAreaColumn();
        $column->DataField = "content";
        $column->HeaderText = $_L['Question Content'];
        $grid->MasterTable->AddColumn($column);

        $column = new GridOptionsColumn();
        $column->DataField = "tree_list";
        ...
        $tree_list = implode(",", $tree_arr);
        $column->AddOption($tree_list);
        $grid->MasterTable->AddColumn($column);
...
//        $ui->assign('_url_import', "tree/import_csv");
        ...
        $ds = new MySQLiDataSource($db_con);
        $ds->SelectCommand = "select id,name from tree";
        $ds->UpdateCommand = "update tree set name='@name' where id = @id";
        $ds->DeleteCommand = "delete from tree where id=@id";
        $ds->InsertCommand = "insert into tree (name) values ('@name');";
